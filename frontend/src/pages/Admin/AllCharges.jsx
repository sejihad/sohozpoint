...
const AllCharges = () => {
  ...
  const { loading, error, success } = useSelector((state) => state.newCharge);
  const {
    error: updateDeleteError,
    isDeleted,
    isUpdated,
  } = useSelector((state) => state.chargeSingle);
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      dispatch({ type: NEW_CHARGE_RESET });
      resetForm();
      setIsFormOpen(false);
      dispatch(getAdminCharge());
    }
    ...
    if (isDeleted) {
      ...
      dispatch({ type: DELETE_CHARGE_RESET });
      dispatch(getAdminCharge());
    }

    if (isUpdated) {
      toast.success("Charge updated");
      dispatch({ type: UPDATE_CHARGE_RESET });
      dispatch(getAdminCharge());
      resetForm();
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
      dispatch(updateCharge(editId, chargeData));
    } else {
      dispatch(createCharge(chargeData));
    }
  };
  ...
  const handleDelete = () => {
    ...
    if (window.confirm("Are you sure you want to delete this charge?")) {
      dispatch(deleteCharge(charge._id));
    }
  };
