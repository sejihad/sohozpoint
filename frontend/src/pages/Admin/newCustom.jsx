...
import { getAdminLogos } from "../../actions/logoAction"; // Logo actions
...
const NewCustom = () => {
  ...
  const { logos } = useSelector((state) => state.logos); // Logos from state
  ...
  const [selectedLogos, setSelectedLogos] = useState([]); // Selected logo IDs
  ...
  useEffect(() => {
    ...
    dispatch(getTypes());
    dispatch(getBrands());
    dispatch(getAdminLogos()); // Fetch all logos
  ...
  const resetForm = () => {
    ...
    setListItemInput("");
    setColorInput("");
    setSelectedLogos([]); // Reset selected logos
  };
  ...
  const handleLogoSelect = (logoId) => {
    if (selectedLogos.includes(logoId)) {
      // Remove logo if already selected
      setSelectedLogos(selectedLogos.filter((id) => id !== logoId));
    } else {
      // Add logo if not selected
      setSelectedLogos([...selectedLogos, logoId]);
    }
  };

  const handleLogoDeselect = (logoId) => {
    setSelectedLogos(selectedLogos.filter((id) => id !== logoId));
  };
  ...
  const handleSubmit = (e) => {
    ...
    // Send listItems as array properly
    listItems
    ...
    // Send sizes as array properly
    formData.sizes.forEach((size) => {
      data.append("sizes", size);
    });

    // Send colors as array properly
    ...
    // Add selected logos to form data
  ...
  return (
    <div className="min-h-screen container bg-gray-50">
      <MetaData title="Create New Custom Product" />
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mt-2">
              Add product details and select logos to associate with this
              product
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      {selectedLogos.map((logoId) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleLogoDeselect(logoId)}
              ...
              >
                {loading ? (
                  ...
                  <>
                    <FiPlus size={18} /> Create Custom Product
                  </>
                )}
              </button>
            </form>
